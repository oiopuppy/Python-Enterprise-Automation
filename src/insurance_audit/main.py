...
def run_audit() -> int:
    ...
    Returns:
        0 = 成功, 1 = 数据问题, 2 = 系统错误
    """
    audit_logger.log("AUDIT_START", "开始执行理赔数据审计流程")
    ...
    try:
        ...
        audit_logger.log("VALIDATION_FAILED", str(e))
        return 1
...
def main() -> NoReturn:
    ...
    try:
        ...
        audit_logger.log("DATA_ERROR", str(e))
        exit_code = 1
    except SecurityException as e:
        logger.critical(f"🔒 安全异常: {e}")
        audit_logger.log("SECURITY_ALERT", str(e))
        exit_code = 3
    except SystemException as e:
        logger.error(f"❌ 系统异常: {e}")
        audit_logger.log("SYSTEM_ERROR", str(e))
        exit_code = 2
    except InsuranceAuditError as e:
        logger.error(f"❌ 审计异常 [{e.error_code}]: {e}")
        audit_logger.log("UNKNOWN_ERROR", str(e))
        exit_code = 9
    except Exception as e:
        logger.critical(f"💥 未预期异常: {e}")
        audit_logger.log("CRITICAL_ERROR", str(e))
        exit_code = 99
    finally:
        audit_logger.log(
            "AUDIT_COMPLETE",
